...
include("DEFAssignStartingPlotsW8");
...
function AddRivers()
	...
	local passConditions = {
		...
		function(plot)
			...
			local plotsPerRiverEdge = 12 --GameDefines["PLOTS_PER_RIVER_EDGE"];
		...
		function(plot)
			local area = plot:Area();
			local plotsPerRiverEdge = 12 --GameDefines["PLOTS_PER_RIVER_EDGE"];
			--local plotsPerRiverEdge = 10; -- possibly randomize this number????
	...
	for iPass, passCondition in ipairs(passConditions) do
		...
		if (iPass <= 2) then
			riverSourceRange = 3 --GameDefines["RIVER_SOURCE_MIN_RIVER_RANGE"];
			--riverSourceRange = 2 --Map.Rand(1, "River Min River Range"); --Lower number =  more rivers, higer = less
			print("-"); print("River Min River Range: ", riverSourceRange);
			seaWaterRange = 2 --GameDefines["RIVER_SOURCE_MIN_SEAWATER_RANGE"];
			--seaWaterRange = 5 --Map.Rand(1, "River Min Sea Range"); --Lower number =  more rivers, higer = less
			print("-"); print("River Min Sea Range: ", seaWaterRange);
		else
			riverSourceRange = 1 --(GameDefines["RIVER_SOURCE_MIN_RIVER_RANGE"] / 2);
			--riverSourceRange = 2 --Map.Rand(1, "River Min River Range 2"); --Lower number =  more rivers, higer = less
			print("-"); print("River Min River Range 2: ", riverSourceRange);
			seaWaterRange = 1 --(GameDefines["RIVER_SOURCE_MIN_SEAWATER_RANGE"] / 2);
			--seaWaterRange = 3 --Map.Rand(1, "River Min Sea Range 2"); --Lower number =  more rivers, higer = less
			print("-"); print("River Min Sea Range 2: ", seaWaterRange);
		end
...
function AddLakes()
	...
	local lakePlotRand = 80 --Map.Rand(101, "") + 30; --Lower number =  more lakes, higer = less
	print("-"); print("Lake Rand: ", lakePlotRand);
	for i, plot in Plots() do
		if not plot:IsWater() then
